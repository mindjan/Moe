package lt.agmis.Moe.integration.moduletests;
...
import lt.agmis.Moe.integration.testutils.MockRequest;
...
import static lt.agmis.Moe.integration.matchers.IsPositiveInteger.positiveInteger;
...
public class CategoryIT extends AbstractTestNGSpringContextTests
{
	...
	public void setUp() throws Exception {
		mockMvc = webAppContextSetup(wac)
				.defaultRequest(get("/").contextPath("/Moe").servletPath("/api"))
				.build();
		mockRequest = new lt.agmis.Moe.integration.testutils.MockRequest(mockMvc);
    ...
    public static final Object[][] getUnsuportedHttpMethodsForCategoryWs() {
        return new Object[][] {
                {"categories PUT method",
                        put("/Moe/api/categories")},
                {"categories DELETE method",
                        delete("/Moe/api/categories")},
                {"categories/id GET method",
                        get("/Moe/api/categories/{id}", 1)},
                {"categories/id POST method",
                        post("/Moe/api/categories/{id}", 1)}};
    }
    ...
    @Test(dependsOnMethods = {"categoryWsShouldRejectUnsuportedHttpMethodsWith404"})
     public void shouldReturnZeroCategoriesWhenNoCategoriesWereCreated() throws Exception {
        String json =  mockRequest.retrieveJson(get("/Moe/api/categories"));
    ...
    public void creationOfCategoryShouldReturnId() throws Exception {
        String json = mockRequest.submitJsonRetrieveJson(post("/Moe/api/categories"), "{\"name\":\"Programming Languages Category\"}");
    ...
    public void shouldCreateAndRetrieveCategory() throws Exception {
        String jsonWithCreatedCategoryId = mockRequest.submitJsonRetrieveJson(post("/Moe/api/categories"), "{\"name\":\"Programming Languages Category\"}");
        Integer categoryId = JsonPath.read(jsonWithCreatedCategoryId, "$.id");
        String jsonCategoryList = mockRequest.retrieveJson(get("/Moe/api/categories"));
    ...
    public void shouldUpdateCategory() throws Exception {
        String jsonWithCreatedCategoryId = mockRequest.submitJsonRetrieveJson(post("/Moe/api/categories"), "{\"name\":\"Programming Languages Category\"}");
        Integer categoryId = JsonPath.read(jsonWithCreatedCategoryId, "$.id");
        mockRequest.submitJson(put("/Moe/api/categories/{id}", categoryId), "{\"name\":\"Updated Category\"}");
        String jsonCategoryList = mockRequest.retrieveJson(get("/Moe/api/categories"));
        String jsonCategory = extractCategoryJsonFromListJsonWithId(jsonCategoryList, categoryId);
        with(jsonCategory)
                .assertThat("$.id", equalTo(categoryId))
    ...
    public void shouldDeleteCategory() throws Exception {
        String jsonWithCreatedCategoryId = mockRequest.submitJsonRetrieveJson(post("/Moe/api/categories"), "{\"name\":\"Programming Languages Category\"}");
        Integer categoryId = JsonPath.read(jsonWithCreatedCategoryId, "$.id");
        mockRequest.submitRequest(delete("/Moe/api/categories/{id}", categoryId));
        String jsonCategoryList = mockRequest.retrieveJson(get("/Moe/api/categories"));
